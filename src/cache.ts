...
export async function getCachedRange(
  ...
  const { results } = await db
    .prepare(
      `SELECT date_key, data, fetched_at FROM oura_cache WHERE metric = ? AND date_key IN (${placeholders})`,
    )
    .bind(metric, ...dates)
...
export async function setCachedRange(
  ...
  const stmts = entries.map(({ dateKey, data }) =>
    db
      .prepare(
        "INSERT OR REPLACE INTO oura_cache (metric, date_key, data, fetched_at) VALUES (?, ?, ?, ?)",
      )

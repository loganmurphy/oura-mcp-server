...
import { datesInRange, defaultStart, defaultEnd, getCachedRange, setCachedRange } from "../cache"
...
describe("getCachedRange", () => {
  ...
  it("returns a hit for a freshly cached row", async () => {
    const db = createMockD1()
    await setCachedRange(db, "daily_sleep", [{ dateKey: "2026-04-01", data: { score: 85 } }])
  ...
  it("returns a mix of hits and misses", async () => {
    const db = createMockD1()
    await setCachedRange(db, "workouts", [{ dateKey: "2026-04-01", data: { activity: "hiking" } }])
    const result = await getCachedRange(db, "workouts", ["2026-04-01", "2026-04-02"])
    expect(result.hits.size).toBe(1)

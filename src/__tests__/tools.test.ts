...
const ACTIVITY_TOOL_NAMES = ["oura_daily_activity", "oura_workouts", "oura_daily_stress"]

function hasValidSchema(tool: ToolDef) {
  return tool.inputSchema.type === "object" && typeof tool.inputSchema.properties === "object"
}
...
describe("date-range tools", () => {
  it("all tools have start_date and end_date properties", () => {
    for (const tool of ALL_TOOLS) {
      ...
      expect(tool.inputSchema.properties["end_date"], `${tool.name} missing end_date`).toBeDefined()
    }
  })

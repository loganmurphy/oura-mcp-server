...
async function main(): Promise<void> {
  ...
  const listResult = spawnSync(
    "npx", ["wrangler", "kv", "key", "list", "--namespace-id", kvId],
  ...
  if (!(await confirm("Revoke all OAuth tokens? Claude will re-authenticate on next use.", false))) {
  ...
  for (const key of keys) {
    const result = spawnSync(
      "npx", ["wrangler", "kv", "key", "delete", "--namespace-id", kvId, key.name],
      { stdio: ["ignore", "pipe", "pipe"], encoding: "utf8",
        env: { ...process.env, CLOUDFLARE_ACCOUNT_ID: accountId } },
    );
    if (result.status !== 0) {
  ...
  console.log();
  if (failed > 0) {
    warn(`${failed} token(s) could not be deleted — try again or clear them in the Cloudflare dashboard.`);
  } else {
